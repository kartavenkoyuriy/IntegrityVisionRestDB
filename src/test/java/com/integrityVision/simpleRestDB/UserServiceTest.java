...
public class UserServiceTest {
    ...
    public void shouldCorrectInsertNewUser() throws Exception{
        ...
        when(userRepository.findOne(userToInsert.getId())).thenReturn(userToInsert);
        User userAfterGetById = userService.getUserById(userToInsert.getId());

        //Verify
        assertEquals(userToInsert, userAfterTestInsert);
        assertEquals("1", userAfterTestInsert.getLogin());

        assertEquals(userToInsert, userAfterGetById);
